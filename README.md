...
8. Сохранение последнего проекта -> Вам не надо будет переключаться на ваш проект в самом начале.
9. Отсутствие лишних библиотек -> Всё только необходимое.
10. Очень легкий в обучении -> Не нужно много времени для изучения.
...
*Для корректной работы программы, надо использовать:
- DISCORD_ID_CONFIG = "123456789012345678" -> Нужен для Discord Rich Presence
- VERSION_REALCODE = "3.7.0" -> Версия RealCode
- DOWNLOAD_URL = "https://github.com/Kish-Mish122/RealCode/releases/latest" -> Ссылка на релизы для обновления программы
- GITHUB_VERSION_URL_CONFIG = "https://api.github.com/" -> API для получнеия конфига версий
- GITHUB_VERSION_MIN = "https://github.com" -> Ссылка на минимальную версию (JSON)
- GITHUB_TOKEN = "TOKEN_GITHUB" -> Токен (не обязательно, но для увеличения числа попыток для API)
- FORMSPREE_ID = "https://formspree.io/f/your_form_id" -> Ссылка на FormSpree для поддержки
- MIN_REALCODE_VERSION = "3.4.0" -> Минимальная версия RealCode для поддержки (локальная верися)
- PLUGIN_URL_CONF = "https://raw.githubusercontent.com/Kish-Mish122/RealCode-plugins/main/plugins.json" -> Ссылка для плагинов (JSON)
