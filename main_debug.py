...
# ========== ОСНОВНОЕ ПРИЛОЖЕНИЕ ==========
# Найдите в main_debug.py класс CodeEditorApp
# Добавьте print в __init__ методе:

class CodeEditorApp:
    def __init__(self, root):
        print("DEBUG: Начало __init__")
        self.root = root
        print("DEBUG: root установлен")
        self.config = load_config()
        print("DEBUG: config загружен")
        self.root.title(APP_NAME)
        print("DEBUG: title установлен")
        
        try:
            if os.path.exists("icon.ico"):
                self.root.iconbitmap("icon.ico")
                print("DEBUG: иконка установлена")
        except:
            print("DEBUG: иконка не установлена")
            pass
        
        print("DEBUG: создание Discord...")
        self.discord = None
        self.updater = UpdateChecker(self)
        print("DEBUG: Updater создан")
        self.highlighter = None
        self.linter = None
        self._dialog_open = False
        print("DEBUG: начальные переменные установлены")
        
        self.app_dir = get_app_dir()
        print(f"DEBUG: app_dir = {self.app_dir}")
        
        print("DEBUG: создание папок...")
        ensure_directories(self.app_dir)
        print("DEBUG: папки созданы")
        
        print("DEBUG: загрузка конфига...")
        self.config = load_config()
        print("DEBUG: конфиг загружен")
        
        print("DEBUG: создание виджетов...")
        ...
        print("DEBUG: переменные установлены")
        
        print("DEBUG: _setup_window...")
        self._setup_window()
        print("DEBUG: _setup_window done")
        
        print("DEBUG: _create_menu...")
        self._create_menu()
        print("DEBUG: _create_menu done")
        
        print("DEBUG: _create_widgets...")
        self._create_widgets()
        print("DEBUG: _create_widgets done")
        
        print("DEBUG: _bind_global_shortcuts...")
        self._bind_global_shortcuts()
        print("DEBUG: _bind_global_shortcuts done")
        
        print("DEBUG: _init_discord...")
        self._init_discord()
        print("DEBUG: _init_discord done")
        
        print("DEBUG: загрузка проекта...")
        ...
        print("DEBUG: проект загружен")
        
        self.original_stdout = sys.stdout
        self.original_stderr = sys.stderr
        sys.stdout = self
        sys.stderr = self
        print("DEBUG: stdout перенаправлен")
        
        self.plugin_manager = PluginManager(self)
        self.plugin_manager.load_plugins()
        print("DEBUG: плагины загружены")
        
        threading.Thread(target=self._check_updates_thread, daemon=True).start()
        print("Привет, Юзер! Удачного кодинга!")
        print("DEBUG: __init__ завершен")
